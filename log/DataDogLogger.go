...
func (c *DataDogLogger) Open(ctx context.Context, correlationId string) error {
	if c.IsOpen() {
		return nil
	}

	err := c.client.Open(ctx, correlationId)
	if err == nil {
...
//   - correlationId 	(optional) transaction id to trace execution through call chain.
//   - Returns error or nil no errors occured.
func (c *DataDogLogger) Close(ctx context.Context, correlationId string) error {
...
func (c *DataDogLogger) Save(ctx context.Context, messages []clog.LogMessage) error {
	...
	return c.client.SendLogs(ctx, "datadog-logger", data)
}
