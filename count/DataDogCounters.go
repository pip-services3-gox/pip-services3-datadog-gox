...
func (c *DataDogCounters) Open(ctx context.Context, correlationId string) error {
	if c.opened {
		return nil
	}

	c.opened = true
	return c.client.Open(ctx, correlationId)
}

//  Close - closes component and frees used resources.
//   - correlationId 	(optional) transaction id to trace execution through call chain.
//   Returns  error or null no errors occured.
func (c *DataDogCounters) Close(ctx context.Context, correlationId string) error {
	c.opened = false

	return c.client.Close(ctx, correlationId)
}
...
func (c *DataDogCounters) Save(ctx context.Context, counters []ccount.Counter) error {
	...
	err := c.client.SendMetrics(ctx, "datadog-counters", metrics)
	if err != nil {
		c.logger.Error(ctx, "datadog-counters", err, "Failed to push metrics to DataDog")
	}
	return err
}
